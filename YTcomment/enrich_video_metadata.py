...
SOURCE_CSV = 'YTcomment/YT_merge.csv'  # 输入原表
OUT_CSV = 'YTcomment/YT_Final.csv'  # 输出增强表
...
def enrich():
    ...
    with open(SOURCE_CSV, 'r', encoding='utf-8', errors='ignore') as f:
        ...
        new_fields = ['duration_seconds','view_count','category_id','category_title','topic_categories','default_language','default_audio_language','channel_id','channel_country','ratio']
        # 确保 ratio 放在 video_comment_final 后：重新构建字段顺序
        base = fieldnames or []
        out_fields = []
        for col in base:
            out_fields.append(col)
            if col == 'video_comment_final' and 'ratio' not in out_fields:
                out_fields.append('ratio')
        ...
        with open(OUT_CSV, 'w', newline='', encoding='utf-8') as out_f:
            ...
            for row in reader:
                ...
                vc = row.get('video_comment','')
                vf = row.get('video_comment_final','')
                try:
                    vc_i = int(vc)
                    vf_i = int(vf)
                    row['ratio'] = f"{(vc_i / vf_i):.6f}" if vf_i > 0 else ''
                except:
                    row['ratio'] = ''
            ...
            def ratio_key(r):
                try:
                    return float(r.get('ratio', '') or 0)
                except:
                    return 0.0
            buffered_rows.sort(key=ratio_key, reverse=True)
